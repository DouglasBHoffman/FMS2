...
\    ['] <free self :apply
 ...
 :m :remove ( idx -- elem )
    dup self :at \ can't use eself because :at is not yet defined
    swap eself :delete ;m
 ...
 :m :each ( -- elem true | false)
    ...
    if 1  current-idx +!  ^elem eself :@elem true else drop eself :uneach false then ;m
 ...
 :m :+ ( array -- ) \ concatenate given array to the end of this array
    >r begin
       ...
       while 
         eself :add
       repeat r> :uneach ;m
 :m :last ( -- n ) #elems @ 1- eself :at ;m
 
 :m :search ( n -- idx true | false ) 
    eself :size 0 ?do dup i eself :at =
          if drop i true unloop exit then loop drop false ;m

 :m :first ( -- n ) 0 eself :at ;m
 :m :second ( -- n ) 1 eself :at ;m
