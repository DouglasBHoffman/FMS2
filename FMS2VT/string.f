...
: :remove-extra-chars ( c str-obj -- newstr )
   ...
   obj string :size 0 ?do i obj string :at dup c =
                    if last-char-was-c? 0= if newstr :ch+ true to last-char-was-c? else drop then
                    else newstr :ch+ false to last-char-was-c?
                    then
                loop
                last-char-was-c? if newstr string :size 1- newstr string :resize then
                newstr ;
...
: :split ( char str-obj -- 1-arry-obj )
   ...
   while
    ...
    obj :@sub heap> string arr array :add
    dup to strt
    obj :end ! obj :start !
   repeat
   obj :start @ obj string :at c =
     if 1 obj :start +! then
    obj string :size obj :end !
    obj :@sub heap> string arr array :add
   arr ;
\ [then]
