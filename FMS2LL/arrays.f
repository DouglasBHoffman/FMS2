	 ...
	 if \ we have a range
	   2drop refilling-parse-name
	     ...
	     if
	       1+ ( end-of-range )
	       list :last 1+ ?do i list :add loop 2drop
	     else true abort" invalid integer in i{...}"
	     then
     else
       >integer if ( n ) list :add
            else true abort" invalid integer in i{...}"
...
: dictf{ ( n -- fp-array )
  dict> farray
	...
	WHILE
	 ( obj addr len )
	     >float if ( obj ) ( R: r ) dup :add
	            else ( obj ) true abort" invalid number in f{...}"
	            then
	REPEAT ( obj addr len ) 2drop ;
...
: dict${ ( -- array )
  >in @ 0 locals| cnt in |
  begin
    refilling-parse-name dup 0= 2dup S" }" compare
  while
    2drop cnt 1+ to cnt
  repeat 2drop
  in >in !
...
dict${ Saturday Sunday Monday Tuesday Wednesday Thursday Friday }
constant d
 
0 d :at :. Saturdayok
1 d :at :. Sundayok
7 d :at \ => aborted, array index out of range
-1 d :at \ => aborted, array index out of range
d :each . :. -1 Saturdayok
d :each . :. -1 Sundayok
d :each . :. -1 Mondayok
d :uneach ok
...
\ the following fails because input is not all on the same line
dict${ Saturday Sunday
...
7 dict${ Saturday Sunday Monday Tuesday Wednesday Thursday Friday }
constant d
 
0 d :at :. Saturdayok

' :. d :apply SaturdaySundayMondayTuesdayWednesdayThursdayFriday

[: swap :@ rot :@ compare 1 < ;] d sort$s ok
' :. d :apply FridayMondaySaturdaySundayThursdayTuesdayWednesdayok
