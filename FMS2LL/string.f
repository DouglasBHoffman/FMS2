...
\ insert the given text at the front of the string
 :m :prepend ( addr len -- )
    self :reset  self :insert ;m
...
:m :chinsert ( c -- ) pad c!  pad 1 self :insert ;m 

:m :first ( -- c ) 0 eself :at ;m
:m :second ( -- c ) 1 eself :at ;m
:m :last ( -- c ) eself :size 1- eself :at ;m
...
:m :remove-extra-chars ( c -- newstr )
   0 0 heap> string true locals| last-char-was-c? newstr c  |
   eself :size 0 ?do i eself :at dup c =
...
:m :split ( char -- 1-arry-obj )
   0 heap> array 0 locals| strt arr obj c |
   eself :reset
   c eself :remove-extra-chars to obj
   obj :reset
   begin
     c obj :chsearch
   while
